 ...
 * ------------------------
 * The Hiscores board — a sortable ranking of characters from the social
 * backend (device-local by default, a shared endpoint when configured). Pure
 * presentation: it reads from the backend and renders; it changes nothing.
 */

import type { Content } from "../core/types.ts";
import { getSocial, type HiscoreEntry } from "./social.ts";
...
export class HiscoresUI {
  ...
  constructor(root: HTMLElement, private content: Content) {
    ...
    const note = this.backdrop.querySelector(".hiscores-note") as HTMLElement;
    let remote = false;
    try { remote = !!localStorage.getItem("varath.social.endpoint"); } catch { /* ignore */ }
    note.textContent = remote
      ? "Shared across all of Varath."
      : "Ranking the heroes on this device. (Connect a server for a world board.)";
  }
  ...
  async show(youName: string): Promise<void> {
    ...
    const list = await getSocial(this.content).hiscores();
    for (const e of list) e.you = e.name === youName;
    this.entries = list;
