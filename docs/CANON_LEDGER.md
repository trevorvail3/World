...
- ~~Our `knucklestone_dagger/helm/shield/mail` have no canon equivalent~~ —
  **RESOLVED.** The placeholders were retired (471 → **467** items); forging now
  produces the canon Knucklestone set (`dagger_1`/`helm_1`/`shield_1`/`armor_1`).
- **Canon gear is now equippable.** `EquipSlot` expanded to the canon wearable
  slots (mainhand/offhand/helmet/armor/legs/boots/ring/necklace/cape); equipping
  reads `ItemDef.slot`, enforces `GEAR_TIER_REQS` by combat level, and handles
  two-handed weapons (evicts the off-hand). Combat consumes worn weapon
  `acc`/`dmg`/`speed`/`attackStyle` and summed armour `def`. Equipment tab shows
  all nine slots with live Acc/Dmg/Def totals.
